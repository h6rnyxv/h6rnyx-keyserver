...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!key || typeof key !== "string") {
      return NextResponse.json({ valid: false, message: "Se requiere una key válida" }, { status: 400 });
    }
    ...
    if (!data.is_active) {
      return NextResponse.json({ valid: false, message: "Key desactivada" }, { status: 403 });
    }
